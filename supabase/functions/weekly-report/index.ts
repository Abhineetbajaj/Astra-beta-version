...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt =
      ...
      'Write a Premium weekly deep-dive report — 4 short paragraphs, each 2-3 sentences, each grounded in a ' +
      'different specific fact (do not repeat the same placement across paragraphs unless genuinely relevant to both):\n' +
      ...
      'not against it.\n' +
      'Write each paragraph on its own line, no headers or markdown — just four grounded paragraphs of prose. ' +
      'Do not invent transits or timing beyond what the given facts support.'
    ...
    const body = await generateWithGemini({
      systemInstruction:
        ...
        'reading. Every claim traces to a named planet, sign, house, nakshatra, dignity, or dasha lord. You have ' +
        `real technical opinions, not just encouragement, and you name real friction alongside real strength. ${CLASSICAL_VOICE_DIRECTIVE}`,
      prompt,
      temperature: 0.9,
    ...
    const { data: inserted, error: insertError } = await admin
      .from('weekly_reports')
      .insert({ user_id: user.id, birth_profile_id: birthProfileId, week_start: weekStart, facts_used: facts, body })
      .select()
      .single()
    if (insertError) {
