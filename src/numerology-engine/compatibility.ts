...
function classifyPair(a: number, b: number): CompatibilityVerdict {
  ...
  // Compatibility lists aren't guaranteed symmetric (a number's own "most harmonious" list may
  // not mention the other number even when the other's list mentions it back) — check both ways.
...
// Three of the numbers most commonly used for numerology matching (per the research this feature
// is based on: "Life Path/Soul Urge matching... a direct analog and a strong sharing hook") — an
// honestly-scoped subset, same convention as the astro-engine's guna.ts covering 3 of 8 kutas
// rather than claiming a complete classical system.
