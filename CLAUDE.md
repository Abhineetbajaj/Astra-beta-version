...
scripts/
  generateLearnPages.mjs  post-build step (last stage of `npm run build`, after `vite build`) —
                          reads src/data/numerologyMeanings.ts + glossary.ts directly (Node's
                          native TS type-stripping, no compile step) and writes plain static HTML
                          to dist/learn/** (12 numerology pages, 51 glossary pages, 3 hub/index
                          pages), then overwrites dist/sitemap.xml with the full 69-URL list. Every
                          /learn/* URL MUST go through this file's learnHref() helper — Render only
                          serves the real generated page for the trailing-slash form, see
                          "SEO/GEO/AEO baseline" in the setup checklist below.
...
| Custom domain | ✅ **Live**: `astraastroconsultancy.com` is the canonical domain (A record → Render's `216.24.57.1`; `www` CNAME → Render, 301s to the bare domain). `SITE_URL` secret, Supabase Auth's Site URL/Redirect URLs allowlist, and the Google OAuth client's Authorized JavaScript origins were all updated to match — skipping any of those three breaks login on the new domain specifically, not the old `onrender.com` one. **`astraastroconsultancy.in` was also purchased but is deliberately NOT set up to redirect yet** — user's call, parked as-is for now; the correct fix when revisited is GoDaddy's Forward Domain (301, no masking) to the `.com` root, not a second live Render domain (that would split SEO ranking signal across two URLs serving identical content). |
| SEO/GEO/AEO baseline | ✅ `robots.txt` explicitly allows GPTBot/ClaudeBot/PerplexityBot/OAI-SearchBot/Googlebot; `index.html` has canonical link, Open Graph/Twitter tags, and Organization+WebSite JSON-LD (site name "Astra" set explicitly, independent of the domain). Domain property verified in Google Search Console. **`/learn/*` static content pages are now built and live** (`scripts/generateLearnPages.mjs`, runs as the last step of `npm run build` — order matters, `vite build` empties `dist/` on every run): 12 numerology-number pages + 51 glossary-term pages + 3 hub/index pages (`/learn/`, `/learn/numerology/`, `/learn/glossary/`), reusing `numerologyMeanings.ts`/`glossary.ts` directly via Node's native TS type-stripping (no duplication, no build-step dependency). Real per-page `<title>`/description/canonical/OG tags, FAQPage JSON-LD on numerology pages, DefinedTerm JSON-LD on glossary pages. `sitemap.xml` grew from 3 URLs to 69. **Verified live against production, not just locally** — a first deploy shipped with no-trailing-slash URLs (matching `/auth`/`/pricing` convention and what Render's own docs suggested), and a post-deploy `curl` check caught that Render's actual dashboard SPA rewrite serves the real generated page **only** for the trailing-slash form (`/learn/numerology/7/`, not `/learn/numerology/7`) — the no-slash form silently fell through to the SPA shell on every single page. Fixed by routing every `/learn/*` href/canonical/sitemap-loc through one `learnHref()` helper in the generator that enforces the trailing slash; re-verified live afterward. **If this page type is ever extended, keep using `learnHref()` for new links — don't hand-write a `/learn/...` path without it.** ⚠️ **Still open**: the regenerated sitemap needs to be resubmitted in Search Console (user action — needs their Google login); the homepage/app nav has no link to `/learn` yet, so these pages are discoverable via sitemap/direct links but not by clicking through the live app; Rahu Kaal/Choghadiya per-city pages were deliberately deferred (date-dependent content needs a daily-rebuild story — a cron hitting Render's deploy hook, or a GitHub Action — which is a separate infra decision, not a static-generator extension). |
| `RUMIK_API_KEY` / `get-reading-audio` | ⚠️ **Built, not yet verified against the real API.** Rumik Silk TTS narrates the Dashboard daily reading and the Numerology daily reading ("Listen"). Stage 1 stores **no audio at all** — no bucket, no cache table; MP3 bytes stream back through the edge function and live only in the browser tab's memory, because Rumik's storage/caching/commercial-use terms have **not** been confirmed. Since nothing is cached, every fresh request is a paid synthesis, so `tts_usage_daily` caps it at **1 new synthesis per user, per reading type, per UTC day**, claimed atomically before the Rumik call and released if synthesis fails. Frozen config: model `mulberry`, `audio_format: mp3`, **no `speaker`** (none confirmed available on the account — do not guess one), and a fixed narrator `description`. Reading text is sent **unchanged**; this function never generates or alters astrology. A persistent `reading_audio` cache is designed and deliberately deferred pending Rumik's written storage/caching permission. |
...
- **"Listen" (meditation/reflection section) ships text-only in this pass — no audio narration.**
  ...
  **Update:** real TTS now exists elsewhere in the app via Rumik Silk (`get-reading-audio`, see the
  `RUMIK_API_KEY` row above), but it deliberately covers only the Dashboard and Numerology daily
  readings. Meditation tracks are **not** narrated and `audio_url` is still unused — extending
  Rumik to them is a separate, later decision, not an oversight.
