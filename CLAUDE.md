...
| SEO/GEO/AEO baseline | ✅ `robots.txt` explicitly allows GPTBot/ClaudeBot/PerplexityBot/OAI-SearchBot/Googlebot; `sitemap.xml` and `llms.txt` exist; `index.html` has canonical link, Open Graph/Twitter tags, and Organization+WebSite JSON-LD (site name "Astra" set explicitly, independent of the domain). **Deliberately thin, not yet complete**: the sitemap only lists the 3 actually-public routes (`/`, `/auth`, `/pricing`) — everything else is real, chart-specific content behind login, invisible to any crawler by design. The bigger opportunity (public, pre-rendered `/learn/*` content pages — numerology meanings, glossary terms, Rahu Kaal tables — reusing `numerologyMeanings.ts`/`glossary.ts`/`muhurta.ts` as a build-time static-HTML generator, since GPTBot/ClaudeBot/PerplexityBot don't execute JavaScript at all as of 2026) is planned but **not built** — a separate, larger piece of work, not scope-crept into this domain migration. ⚠️ **Also not done**: adding `astraastroconsultancy.com` as a **Domain**-type property in Google Search Console (now that DNS is actually owned, unlike the earlier `onrender.com` URL-prefix-only situation) — needs a TXT record at GoDaddy, user action. |
